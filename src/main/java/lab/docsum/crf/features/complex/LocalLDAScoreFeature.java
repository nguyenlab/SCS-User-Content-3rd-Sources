package lab.docsum.crf.features.complex;
...
public class LocalLDAScoreFeature implements Feature<double[],Doc>  {
	String ldaFolder;
	 public LocalLDAScoreFeature(String ldaFolder) {
	...
	public double[] extract(Doc doc, Object... args) {
		...
		IOUtils.readLines(ldaFolder +"/"+ name).forEach(line-> {String[]cols = line.split("\\s+"); ldaWordDist.put(cols[0], Double.valueOf(cols[1]));});

		for (int i = 0; i < doc.getSentences().size(); i++) {
			Set<String> tokens = getTokens.apply(doc.getSentences().get(i));
			List<String> words = tokens.stream().filter(tk->!NLPUtils.stopwords.contains(tk)).collect(Collectors.toList());
	...
	@Override
	public String name() {
		return "Local_LDA_Score";
	}
	
	private double localLDAScore(List<String> words, Map<String, Double> wordDist){
		if (words.isEmpty()) return 0.0;
		return words.stream().mapToDouble(w->wordDist.getOrDefault(w,0.0)).average().getAsDouble();
	}
	...
	private String getName(String ldaFolder, String docSource){
		...
		if(!folder.exists()){
			throw new IllegalStateException("LDA folder not found. "+ldaFolder);
		}
