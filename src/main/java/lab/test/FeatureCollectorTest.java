...
import lab.docsum.crf.main.GoogleSearchDocLoader;
import lab.docsum.misc.Doc;

public class FeatureCollectorTest {
	// static int nThreads = 40;
	static final Logger logger = Logger.getLogger(FeatureCollectorTest.class
			.getName());
				...
				.forEach(docCom -> {
					// single feature
						...
						for (int i = 0; i < docCom.size(); i++) {
							Doc doc = docCom.get(i);
							String docId = doc.getSource() + "." + doc.getTag();
							List<Feature> singleFeatures = FeatureProviderTest.inst.googleSearchFeatures
									.getFeatureSets().get(i);
							for (Feature feature : singleFeatures) {
								File ftDir = new File(allFeatureDir, feature
										.name());
