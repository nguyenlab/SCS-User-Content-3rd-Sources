...
public class JaroWinklerDistance {
     ...
     * sequences.
     */
    public double distance(List<String> cSeq1, List<String> cSeq2) {
     ...
     * sequences.
     */
    public double proximity(List<String> cSeq1, List<String> cSeq2) {
        int len1 = cSeq1.size();
        int len2 = cSeq2.size();
        // if (len1 == 0)
        ...
        for (int i = 0; i < len1; ++i) {
            ...
            for (int j = start; j < end; ++j) {
                ...
                if (!cSeq1.get(i).equals(cSeq2.get(j))) {
                    continue;
                }
                matched1[(i)] = true;
                matched2[(j)] = true;
        ...
        int max = Math.min(mNumChars, Math.min(cSeq1.size(), cSeq2.size()));
        int pos = 0;
        while (pos < max && cSeq1.get(pos).equals(cSeq2.get(pos))) {
            ++pos;
        }
        if (pos == 0) {
            return weight;
    ...
    public double jaro(List<String> cSeq1, List<String> cSeq2) {
        int len1 = cSeq1.size();
        int len2 = cSeq2.size();
        if (len1 == 0) {
            return len2 == 0 ? 1.0 : 0.0;
